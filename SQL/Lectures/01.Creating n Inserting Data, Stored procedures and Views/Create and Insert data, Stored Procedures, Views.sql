...
-- show all records, age equals to 33 
SELECT * FROM BANK_DETAILS WHERE AGE=33;

-- show all records, age equals to 60
SELECT * FROM BANK_DETAILS WHERE AGE = 60;

-- show all records, age equals to 60 and job is retired
SELECT * FROM BANK_DETAILS WHERE AGE = 60 AND JOB='RETIRED';

-- show all records where education is unknown or marital is single
SELECT * FROM BANK_DETAILS WHERE EDUCATION = 'UNKNOWN' OR MARITAL = 'SINGLE';

-- show all records where education is unknown or marital is single, and balance less than 500
...
-- show all records in descending order
SELECT * FROM BANK_DETAILS ORDER BY AGE DESC;

/* QUESTIONS */

-- find out the sum of balance
SELECT SUM(BALANCE) FROM BANK_DETAILS;

-- find out the average of balance
SELECT AVG(BALANCE) FROM BANK_DETAILS;

-- find out the minimum of balance
...
-- find out the maximum of balance
...
-- show all the records who are having loan
SELECT * FROM BANK_DETAILS WHERE LOAN='YES';

-- find out average balance for all people whose job role is admin
SELECT AVG(BALANCE) FROM BANK_DETAILS WHERE JOB='ADMIN.';

-- find out the people without job whose age is below 45
SELECT * FROM BANK_DETAILS WHERE JOB='UNKNOWN' AND AGE<45;

-- find out the records whose education is primary and person is jobless
SELECT * FROM BANK_DETAILS WHERE JOB='UNKNOWN' AND EDUCATION='PRIMARY';

-- find out the records whose bank account is having a negative balance
SELECT * FROM BANK_DETAILS WHERE BALANCE<0;

-- find out the records who is not having house at all along with there balance
...
-- while calling same query again and again to view the same results, it's better to write a stored procedure
...
-- If we want the results based on 1 condition, we have to use 'IN' parameter  
-- while creating stored procedure.
DELIMITER &&
CREATE PROCEDURE AVG_BAL_JOBROLE1(IN JOBROLE_TYPE VARCHAR(30))
BEGIN 
	SELECT AVG(BALANCE) FROM BANK_DETAILS WHERE JOB = JOBROLE_TYPE;
END &&

-- calling stored procedure
CALL AVG_BAL_JOBROLE1('MANAGEMENT')

-- If we have multiple conditions, use multiple 'IN' parameter  
DELIMITER &&
...
-- calling stored procedure with multiple condition values
CALL SEL_EDU_JOB1('TERTIARY', 'RETIRED');


/* VIEWS */

-- Views are created in order to store your queries as virtual table. If there 
-- are a lot of columns and we don’t want to use all columns and make the table 
-- simpler, we can create a view and reuse it repeatedly.

-- create view
CREATE VIEW BANK_VIEW AS SELECT AGE,JOB,MARITAL,BALANCE,EDUCATION FROM BANK_DETAILS;

-- show the records in view
